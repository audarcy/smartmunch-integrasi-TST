import React, { useState } from 'react';
import axios from 'axios';
import { useNavigate } from 'react-router-dom';
import querystring from 'querystringify';
import { Link } from 'react-router-dom';
import {
  Flex,
  Box,
  FormControl,
  FormLabel,
  Input,
  InputGroup,
  InputRightElement,
  Stack,
  Button,
  Heading,
  Text,
  useColorModeValue,
} from '@chakra-ui/react'
import { ViewIcon, ViewOffIcon } from '@chakra-ui/icons'
import iconImage from './menu.png'; 

const SignIn = () => {
  const navigate = useNavigate();
  const [showPassword, setShowPassword] = useState(false)
  const [username, setUsername] = useState('');
  const [password, setPassword] = useState('');
  const [error, setError] = useState('');

  const handleUsernameChange = (e) => {
    setUsername(e.target.value);
  };

  const handlePasswordChange = (e) => {
    setPassword(e.target.value);
  };

  const signInToApis = async () => {
    try {
      const apiUrlLogin1 = 'https://menurecommend.azurewebsites.net/token';
      const apiUrlLogin2 = 'https://loanrecommendationapi.azurewebsites.net/login';

      const dataForApi = querystring.stringify({
          username: username,
          password: password,

      });

      const [response1, response2] = await Promise.all([
        axios.post(apiUrlLogin1, dataForApi, {
          headers: {
            'Content-Type': 'application/x-www-form-urlencoded',
          },
        }),
        axios.post(apiUrlLogin2, dataForApi, {
            headers: {
              'Content-Type': 'application/x-www-form-urlencoded',
            },
        }),
      ]);

      if (response1.data.access_token || response2.data.acess_token) {
        const token1 = response1.data.access_token;
        const token2 = response2.data.access_token;

        sessionStorage.setItem('token1', token1);
        sessionStorage.setItem('token2', token2);
        sessionStorage.setItem('username', username);

        navigate('/home');
      } else {
        console.log(response1.data, response2.data);
        alert('Login failed. Please try again.');
      }
    } catch (error) {
      if (error.response) {
        if (error.response.status === 422) {
          setError('Invalid username or password combination.');
        } else {
          console.log(error);
          setError('An error occurred. Please try again.');
        }
      } else if (error.request) {
        setError('An error occurred while sending the request.');
      } else {
        console.log(error);
        setError('An error occurred. Please try again.');
      }
    }
  };

  const getUser = async () => {
    const storedToken1 = sessionStorage.getItem('token1');
  
    try {
      const response = await axios.get(
        'https://menurecommend.azurewebsites.net/signin_user/me',
        {
          headers: {
            Authorization: `Bearer ${storedToken1}`,
          },
        }
      );
      console.log(response.data.id);
      sessionStorage.setItem('userId', response.data.id);
      const storedID = sessionStorage.getItem('userId');
      console.log("ID:" + storedID);

      // Assuming the response.data contains the user details including the ID
  
      // Make sure to replace 'id' with the actual key in the response data that represents the user ID
    } catch (error) {
      if (error.response) {
        if (error.response.status === 422) {
          setError('Invalid username or password combination.');
        } else {
          console.log(error);
          setError('An error occurred. Please try again.');
        }
      } else if (error.request) {
        setError('An error occurred while sending the request.');
      } else {
        console.log(error);
        setError('An error occurred. Please try again.');
      }
    }
  };

  const handleSubmit = async (e) => {
    e.preventDefault();
    signInToApis();
    getUser();
    
  };

  return (
    <Flex
      minH={'100vh'}
      align={'center'}
      justify={'center'}
      bg={useColorModeValue('gray.50', 'gray.800')}>
      <Stack spacing={8} mx={'auto'} maxW={'lg'} py={12} px={6} >
        <Stack align={'center'}>
          <Heading fontSize={'4xl'} color={'#018ABE'}>Welcome to SmartMunch</Heading>
          <Text fontSize={'lg'} color={'gray.600'}>
            Your Personalized Food and Loan Recommendation
          </Text>
        </Stack>
        <Box
          rounded={'lg'}
          bg={useColorModeValue('white', 'gray.700')}
          boxShadow={'lg'}
          p={8} 
          align={'center'}
          maxW={'xl'}>
          <img src={iconImage} style={{ width: '120px', height: '120px', marginBottom: '10px' }} alt="Icon" />
          <Stack spacing={4}>
            <Heading fontSize={'2xl'} color={'#018ABE'}>Login</Heading>
          <form onSubmit={handleSubmit}>
          <FormControl mb={4} isRequired>
              <FormLabel color={'#018ABE'} fontSize="16px" fontWeight="500" >Username</FormLabel>
              <Input type="text" value={username} onChange={handleUsernameChange} required />
            </FormControl>
            <FormControl id="password" isRequired mb={4}>
              <FormLabel color={'#018ABE'} fontSize="16px" fontWeight="500">Password</FormLabel>
              <InputGroup>
                <Input type={showPassword ? 'text' : 'password'} value={password} onChange={handlePasswordChange} />
                <InputRightElement h={'full'}>
                  <Button
                    variant={'ghost'}
                    onClick={() => setShowPassword((showPassword) => !showPassword)}>
                    {showPassword ? <ViewIcon /> : <ViewOffIcon />}
                  </Button>
                </InputRightElement>
              </InputGroup>
            </FormControl>
            <Stack spacing={10} mt={5}>
            <Button  type="submit"
                bg={'#018ABE'}
                color={'white'}
                _hover={{
                  bg: '#97CADB',
                }}>
                Login
              </Button>
            </Stack>
          </form>
            <Stack align={'center'}>
              <Text fontSize={'md'} color={'gray.600'}>
                {error && <p style={{ color: 'red' }}>{error}</p>}
                <p style={{fontSize:'14px', fontWeight:'400'}} >
                  Don't have an account?{' '}
                  <Link to="/register" style={{color:'#018ABE'}} >
                    Register here
                  </Link>
                </p>
              </Text>
            </Stack>
          </Stack>
        </Box>
      </Stack>
    </Flex>
  )
}

export default SignIn;
